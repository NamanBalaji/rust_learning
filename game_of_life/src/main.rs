...
const CELL_COLOR: Color = Color::RED;
        ...
        while timer::check_update_time(ctx, self.fps) && self.running {
            ...
            for i in 0..GRID_SIZE.0 as usize {
                ...
                for j in 0..GRID_SIZE.1 as usize {
                    ...
                    let neighbors = self.grid[left][j] as u8 + self.grid[left][up] as u8
                    + self.grid[i][up] as u8 + self.grid[right][up] as u8
                    + self.grid[right][j] as u8 + self.grid[right][down] as u8
...
fn main() -> GameResult{
   ...
   let (ctx, event_loop) = ContextBuilder::new("Conway's Game of Life", "namanbalaji")
